...
$( document ).ready(function() {
	...
	$('a.colorboxlink').colorbox({
						iframe:true,
						innerWidth:1025,/* 1024 make it iPad not desktop experience */
						innerHeight:1025,
						fastIframe: false ,
						transition: "fade",
						speed:	100,
						reposition:false,
						top:'0px',
						onOpen:function(){ $('#colorbox').css('display','none'); },
						onComplete:function(){ $('#colorbox').css('display','block'); 
											   $('iframe').css('visibility','visible');
						},
						onClosed:function(){ }
	});

	$('a.colorboxlink').click(function() {
		$('#cboxOverlay').fadeIn();
	});

	if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) && document.querySelector('meta[name="viewport"]')) {
		var viewportmeta = document.querySelector('meta[name="viewport"]');
		$('a.hide-for-large-up').on("click", function() {
			viewportmeta.content = 'width=device-width, minimum-scale=1.0, maximum-scale=1.0, initial-scale=1.0';			
			document.addEventListener('scroll', function () {
				viewportmeta.content = 'width=device-width,  minimum-scale=0.25, maximum-scale=2.0, initial-scale=1';
			});
		});
	}
	...
	//Adding class on window resize and on load
	var $window = $(window),
	$body = $('body');
	...
	//Code for accepting only numbers in phone field
